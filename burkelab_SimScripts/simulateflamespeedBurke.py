...
for i, m in enumerate(list(models.keys())):
    p_list = np.linspace(0,20,gridsz)[1:]
    ...
    vars()['mbr'+str(i)] = mbr
    # ax.plot(p_list, mbr, '-', linestyle='solid', color=colors[i], label=m)
    csv_filename = f'C:\\Users\\pjsin\\Documents\\cantera\\burkelab_SimScripts\\BurkeSongResults_May28\\{m}_data.csv'
...
# ax.plot(dataset1.iloc[:, 0],dataset1.iloc[:, 1],marker='s',color='k', zorder=3, fillstyle='full', linestyle = 'None', label="Burke et al.")
