...
rxns2 = ['H2O + M <=> H + OH + M', 'H + O2 (+M) <=> HO2 (+M)', 'H2O2 (+M) <=> 2 OH (+M)','H + NH2 (+M) <=> NH3 (+M)', '2 NH2 (+M) <=> N2H4 (+M)', 'H + NO (+M) <=> HNO (+M)']
...
for i in range(len(rxns1)):
    plt.figure()
    plotK('test/data/alzuetamechanism_LMRR.yaml',Tlist,1,{'AR':1},rxns1[i],'b','solid','NEW: '+rxns1[i],dirs1[i])
    plotK('test/data/alzuetamechanism.yaml',Tlist,1,{'AR':1},rxns2[i],'k','solid','OLD: '+rxns2[i],dirs2[i])
    plt.legend()
    plt.show()
